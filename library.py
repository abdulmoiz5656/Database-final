...
def BookList():
    ...
    try:
        bookFile = open('image/BookList.txt')
        bookFile.close()
        ...
        bookListRoot.iconbitmap('image/Icon.ico')
        ...
        with open('image/BookList.txt') as bookListFileTreeview:
            ...
            tree = ttk.Treeview(treeFrame, columns=('book id', 'book title', 'book author', 'book status'),
                                show='headings', height=len(bookListTreeview))
        ...
        # Quit Button
        btnQuitBookList = Button(bookListRoot, text='Exit', bg='#f7f1e3', fg='black', font=fonts,
                                 command=bookListRoot.destroy)
...
def SearchBook():
    ...
    try:
        ...
        bookSearchRoot.iconbitmap('image/Icon.ico')
        ...
        # Quit Button
        btnQuitBookSearch = Button(bookSearchRoot, text='Exit', bg='#f7f1e3', fg='black', font=fonts,
                                   command=bookSearchRoot.destroy)
...
def Search():
    ...
    if len(searchID) == 3:
        try:
            dicSearch = {}
            booksInfoSearch = getbooksinfo('image/BookList.txt', dicSearch)
            bookInfo = booksInfoSearch[searchID]
            messagebox.showinfo(title='Book information',
                                message=f'Book Title : {bookInfo[0]}\nBook Author : {bookInfo[1]}\nBook Status : {bookInfo[2]}')
        except KeyError:
...
def BorrowBook():
    ...
    try:
        ...
        bookBorrowRoot.iconbitmap('image/Icon.ico')
        ...
        # Quit Button
        btnQuitBookBorrow = Button(bookBorrowRoot, text='Exit', bg='#f7f1e3', fg='black', font=fonts,
                                   command=bookBorrowRoot.destroy)
...
def Borrow():
    ...
    if len(borrowID) == 3:
        try:
            dicBorrow = {}
            booksInfoBorrow = getbooksinfo('image/BookList.txt', dicBorrow)
            if booksInfoBorrow[borrowID][-1] == 'not taken':
                new_file = []
                with open('image/BookList.txt', 'r') as file:
                    for line in file:
                        if line[: 3] == borrowID:
                ...
                file.close()
                with open('image/BookList.txt', 'w') as file:
                    for line in new_file:
                        file.writelines(line)
                file.close()
                messagebox.showinfo(title='Successfully borrowed',
                                    message=f'The book : {booksInfoBorrow[borrowID][0]}\nwritten by : {booksInfoBorrow[borrowID][1]}\nwith ID : {borrowID}\nsuccessfully borrowed.')
            else:
                messagebox.showerror(title='Already borrowed',
...
def ReturnBook():
    ...
    try:
        ...
        bookReturnRoot.iconbitmap('image/Icon.ico')
        ...
        # Quit Button
        btnQuitBookReturn = Button(bookReturnRoot, text='Exit', bg='#f7f1e3', fg='black', font=fonts,
                                   command=bookReturnRoot.destroy)
...
def Return():
    ...
    if len(returnID) == 3:
        try:
            dicReturn = {}
            booksInfoReturn = getbooksinfo('image/BookList.txt', dicReturn)
            if booksInfoReturn[returnID][-1] == 'taken':
                ...
                messagebox.showinfo(title='Successfully returned',
                                    message=f'The book : {booksInfoReturn[returnID][0]}\nwritten by : {booksInfoReturn[returnID][1]}\nwith ID : {returnID}\nsuccessfully returned.')
            else:
                messagebox.showerror(title='Not borrowed',
...
def AddBook():
    ...
    bookAddRoot.iconbitmap('image/Icon.ico')
...
def Add():
    ...
    if len(bookTitle) != 0:
        if len(bookAuthor) != 0:
            lst1 = []
            bookInfoList = gettitleandauthor('image/BookList.txt', lst1)
            ...
            if bookInfo in bookInfoList:
                ...
                bookID = getlastid('image/BookList.txt')
                if 99 > int(bookID) >= 9:
...
def DeleteBook():
    ...
    try:
        ...
        bookDeleteRoot.iconbitmap('image/Icon.ico')
        ...
        # Quit Button
        btnQuitBookDelete = Button(bookDeleteRoot, text='Exit', bg='#f7f1e3', fg='black', font=fonts,
                                   command=bookDeleteRoot.destroy)
...
def Delete():
    ...
    if len(deleteID) == 3:
        try:
            dicDelete = {}
            booksInfoDelete = getbooksinfo('image/BookList.txt', dicDelete)
            if deleteID in booksInfoDelete.keys():
                ...
                messagebox.showinfo(title='Successfully deleted',
                                    message=f'The book : {booksInfoDelete[deleteID][0]}\nwritten by : {booksInfoDelete[deleteID][1]}\nwith ID : {deleteID}\nhas been successfully deleted.')
            else:
...
library_app.iconbitmap('image/Icon.ico')

# Background Image

backgroundImage = Image.open('image/background.jpg')
